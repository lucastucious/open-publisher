...
<details>
<summary>🚀 𝗖𝗹𝗶𝗰𝗸 𝗵𝗲𝗿𝗲 𝘁𝗼 𝗿𝗲𝘃𝗲𝗮𝗹 𝘁𝗵𝗲 𝗳𝘂𝗹𝗹 𝗩𝟱 𝘂𝗽𝗱𝗮𝘁𝗲 𝗹𝗶𝘀𝘁 𝘀𝗼 𝗳𝗮𝗿</summary>

## What's Coming in V5

We've been hard at work on the next major release of OpenPublisher. Here's a quick look at what's landing in V5 so far:

### Saving & File Safety
- Atomic saves (crash-proof write pipeline with temp files and backup rotation)
- Auto-save on system idle and screen lock
- Smart exit prompts (only warns when you have unsaved changes)
- Previous Versions system with automatic 15-minute snapshots and restore workflows
- Interactive Backup Manager with search, preview, and recovery
- External file change detection and reload prompting

### Editor & Canvas
- Native OS clipboard pasting (bypasses browser permission modals)
- Intelligent paste image separation (images from web content drop in as independent objects)
- Windows system fonts integrated alongside Google Fonts in the font picker
- Native spellcheck with offline English dictionaries
- Full pasteboard scrolling and Shift+Scroll horizontal navigation
- Smooth document pour animation on file open

### Window & Desktop Integration
- Rounded corners, drop shadows, and smooth omnidirectional resizing
- Always-on-Top window pinning
- F11 fullscreen support
- Window position and size remembered across sessions (with multi-monitor safety)
- Single-instance enforcement (opening a second file focuses the existing window)
- Native system tray icon with jump list menu
- Close-to-tray option
- Taskbar progress bars and attention flashing on save/export completion
- Native desktop save notifications
- OS-level global keyboard shortcuts
- MyDockFinder integration with native Win32 menu bridge
- Presentation Preview window with live canvas mirroring and fullscreen mode
- Multi-window lifecycle management with cascading close

### Performance & Stability
- Async IPC (no more UI freezes during saves)
- Uncapped memory limits for large publications (8GB V8 heap, 4GB GPU)
- Renderer crash recovery with automatic restart and work salvage dialog
- Power management (prevents sleep during saves and exports)
- Safe Mode boot with cache reset

### Export & Print
- Export & Print Queue Monitor window
- Process priority elevation during exports
- Native taskbar and Dock export progress bars

### UI & Theming
- Modernized V5 startup screen with adaptive light/dark themes
- Dynamic theme sync across sessions with auto-healing color contrast
- Subpixel font rendering and GPU-accelerated text
- Custom dark scrollbars across all windows
- Redesigned modal dialogs with consistent styling
- Responsive titlebar button layout with progressive collapse

### Developer Tools
- Developer HUD (Ctrl+Shift+D) with live RAM, FPS, and DOM metrics
- Tearaway pop-out HUD window (multi-monitor draggable)
- Real-time IPC traffic sniffer
- Memory footprint chart with auto-zoom and rate monitoring
- Live CSS/UI file watcher with hot reload
- Crash simulator and diagnostic log generation
- Quick actions (Open AppData, Reboot Safe Mode, etc.)

> This is a work-in-progress overview. A full, detailed changelog will accompany the V5 release.
